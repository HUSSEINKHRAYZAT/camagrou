...
install: ## Setup: Install and setup everything (first time)
	...
	@echo "$(YELLOW)🧹 Cleaning up any existing containers...$(NC)"
	@$(DOCKER_COMPOSE) down 2>/dev/null || true
	@docker rm -f $(DB_CONTAINER) $(WEB_CONTAINER) $(PHPMYADMIN_CONTAINER) 2>/dev/null || true
	...
	@echo ""
	@echo "$(YELLOW)⚠️  IMPORTANT: Email is not configured yet!$(NC)"
	@echo "$(BLUE)To enable email verification, run: $(GREEN)make setup-email$(NC)"
	@make urls
...
.PHONY: setup-email
setup-email: ## Setup: Configure email (SMTP) settings
	@echo "$(BLUE)📧 Setting up email configuration...$(NC)"
	@chmod +x setup_email.sh
	@./setup_email.sh
	@echo "$(YELLOW)💡 After setup, rebuild the container: $(GREEN)make rebuild$(NC)"
