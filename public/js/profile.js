(() => {
    ...
    const setAvatar = src => {
        ...
        // Update hidden input with selected avatar path
        if (selectedAvatarInput) {
            selectedAvatarInput.value = src;
        }
    };
    ...
    if (avatarGrid) {
        avatarGrid.addEventListener('click', e => {
            ...
            // Clear the file input since we're using a gallery image
            if (avatarUpload) {
                avatarUpload.value = '';
            }
            ...
            if (selectedFileName) {
                selectedFileName.textContent = 'Selected from your posts';
                selectedFileName.style.color = '#28a745';
            }
        });
    }
