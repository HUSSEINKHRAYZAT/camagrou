...
class User {
    ...
    public function findById($id) {
        ...
        $stmt = $this->pdo->prepare("
            SELECT u.*, 
                   up.profile_picture AS avatar, 
                   up.bio,
                   COALESCE(
                       (SELECT COUNT(*) 
                        FROM likes l 
                        INNER JOIN images i ON l.image_id = i.id 
                        WHERE i.user_id = u.id), 0
                   ) AS like_total
            FROM users u
    ...
    public function updateProfile($userId, $bio, $avatarPath = null) {
        ...
        // If avatarPath is provided, update it; otherwise, only update bio
        if ($avatarPath !== null) {
            $stmt = $this->pdo->prepare("
            ...
            // Only update bio, keep existing profile picture
            $stmt = $this->pdo->prepare("
                INSERT INTO user_profiles (user_id, bio, profile_picture)
                VALUES (?, ?, NULL)
                ON DUPLICATE KEY UPDATE bio = VALUES(bio)
            ");
            return $stmt->execute([$userId, $bio]);
        }
    }
